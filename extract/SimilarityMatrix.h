 ...
 *
 * Each file format must implmenet its own child class for extracting results
 * from its files.
 */
class SimilarityMatrix {
  ...
  public:
    // Constructor.
    SimilarityMatrix(EMatrix *ematrix, int quiet,
        char * cmethod, int x_coord, int y_cood, char * gene1, char * gene2, float th);
    // Desctructor.
    ~SimilarityMatrix();
